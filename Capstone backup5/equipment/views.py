...
def messages_list(request):
    if request.method == "GET":
        ...
        distinct = Message.objects.values('subject').annotate(subject_count=Count('subject'))
        records = Message.objects.filter(subject__in=[item['subject'] for item in distinct]).order_by('-created_date')
...
# distinct = Message.objects.values('subject').annotate(subject_count=Count('subject')).filter(subject_count=1)
# records = Message.objects.filter(subject_in=[item['subject'] for item in distinct])
